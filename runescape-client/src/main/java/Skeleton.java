...
public class Skeleton extends Node {
	...
	static final void drawEntities(int var0, int var1, int var2, int var3) {
				...
				if (Client.cycle >= var4.cycleStart) {
					...
					if (var4.targetIndex < 0) {
						...
						if (var6 == Client.localPlayerIndex) {
							...
							var26 = Client.players[var6];
						}

						if (var26 != null && var26.x >= 0 && var26.x < 13312 && var26.y >= 0 && var26.y < 13312) {
		...
		if (Client.oculusOrbState == 1 && Client.staffModLevel >= 2 && Client.cycle % 50 == 0 && (ModeWhere.oculusOrbFocalPointX >> 7 != Projectile.localPlayer.x >> 7 || GrandExchangeEvents.oculusOrbFocalPointY >> 7 != Projectile.localPlayer.y >> 7)) {
			...
			var19.packetBuffer.method9000(var18);
			var19.packetBuffer.writeByte(var16);
			var19.packetBuffer.method8949(var17);
			var19.packetBuffer.method8961(Client.field601);
		...
		for (var16 = 0; var16 < 5; ++var16) {
			if (Client.field771[var16]) {
				var17 = (int)(Math.random() * (double)(Client.field772[var16] * 2 + 1) - (double)Client.field772[var16] + Math.sin((double)Client.field763[var16] * ((double)Client.field774[var16] / 100.0D)) * (double)Client.field695[var16]);
				if (var16 == 0) {
		...
		if (Client.isLoading) {
			byte var20 = 0;
			int var25 = var20 + NetCache.NetCache_pendingPriorityResponsesCount + NetCache.NetCache_pendingPriorityWritesCount;
			if (var25 == 0) {
